...
{   
    ...
    if (firstScanEstimate){
        std::thread mpc_thread(&MPC::Update, &mpc_, current_state, get_next_input(), std::ref(trajp_.best_cmaes_trajectory_));//(mpc_.Update(current_state, trajp_.best_cmaes_trajectory_);
        mpc_thread.join();
